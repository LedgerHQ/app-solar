...
def test_get_public_key_chaincode_no_confirm(backend):
    ...
    ref_public_key, ref_chain_code = calculate_public_key_and_chaincode(CurveChoice.Secp256k1, path=path, compress_public_key=True)
...
def test_get_public_key_chaincode_confirm_accepted(firmware, backend, navigator, test_name):
    client = SolarCommandSender(backend)
    path = "m/44'/3333'/0'/0/0"
