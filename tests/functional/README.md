...
make clean && make BOLOS_SDK=$<device>_SDK      # replace <device> with one of [NANOSP, NANOX, FLEX, STAX, APEX_P]
exit
```
...
docker run --user "$(id -u)":"$(id -g)" --rm -ti -v "$(realpath .):/app" --privileged -v "/dev/bus/usb:/dev/bus/usb" ledger-app-builder-lite:latest
make clean && make BOLOS_SDK=$<device>_SDK load     # replace <device> with one of [NANOSP, NANOX, FLEX, STAX, APEX_P]
exit
```
...
```
    --device <device>           run the test on the specified device [nanosp,nanox,flex,stax,apex_p,all]. This parameter is mandatory
